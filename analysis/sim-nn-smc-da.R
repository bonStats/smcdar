...
nn_optimise_pre_approx_llhood_transformation <- function(particles, b_s_start, loglike, temp){
  ...
  true_post <- apply(particles, MARGIN = 1, loglike, type = "full_posterior", temp = temp)
  
  len <- ncol(particles)
  
  if(missing(b_s_start)){
    
    approx_post <-apply(particles, MARGIN = 1, loglike, type = "approx_posterior", temp = temp)
    mode_true <- which.max(true_post)
    mode_approx <- which.max(approx_post)
    b_s_start <- rep(0, times = 2 * len)
    b_s_start[1:len] <- particles[mode_approx,] - particles[mode_true,]
  }
  ...
  topt <- function(b_s){
    ...
    approx_post <- apply((particles - b_mat)/exp(s_mat), MARGIN = 1, loglike, type = "approx_posterior", temp = temp)
    
    keep_ll <- is.finite(approx_post)
    
    # Chi-sq discrepancy
    sum( # p * (p/q - 1)^2
      ( exp(approx_post[keep_ll] - 0.5 * true_post[keep_ll] ) - exp(0.5 * true_post[keep_ll]) )^2
    )
    
  }
  
  optim(par = b_s_start, fn = topt, control = list(maxit = 1), method = "BFGS")
